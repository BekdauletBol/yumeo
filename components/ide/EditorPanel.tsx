...
const TABS = [
  { key: 'references' as const, label: 'Refs', accent: 'var(--accent-refs)' },
  { key: 'figures'    as const, label: 'Figs', accent: 'var(--accent-figures)' },
  { key: 'tables'     as const, label: 'Tables', accent: 'var(--accent-tables)' },
  { key: 'templates'  as const, label: 'Template', accent: 'var(--accent-template)' },
  { key: 'mermaid'    as const, label: 'Mermaid', accent: 'var(--text-secondary)' },
  { key: 'latex'      as const, label: 'LaTeX', accent: 'var(--text-secondary)' },
] as const;
...
export function EditorPanel() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        {TABS.map((tab) => {
          const count = materials.filter((m) => m.section === tab.key).length;
