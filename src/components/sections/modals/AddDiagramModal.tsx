...
export function AddDiagramModal({ isOpen, onClose }: AddDiagramModalProps) {
  ...
  const diagramsSection = sections.find(s => s.sectionType === 'diagrams');

  if (!isOpen) return null;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !mermaidCode.trim() || !activeProject || !diagramsSection) return;
    ...
    try {
      ...
      const material = await createMaterialAction({
        ...
        section: 'diagrams',
        sectionId: diagramsSection.id,
        name,
        content: mermaidCode,
        metadata: { fileType: 'mermaid', fileSize: mermaidCode.length },
      });
      ...
      console.error('Failed to add diagram:', err);
      alert(err instanceof Error ? err.message : 'Failed to add diagram');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-semibold">Add Mermaid Diagram</h2>
            <p className="text-xs mt-1" style={{ color: 'var(--text-secondary)' }}>
              Create flowcharts, ERDs, timelines, etc.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Name */}
          <div>
            <label className="block text-sm font-medium mb-2">Diagram Name</label>
            <input
              type="text"
              value={name}
  ...
  C -->|No| B`}
          ...
          <div className="flex gap-3 justify-end pt-4">
            ...
            <button
              type="submit"
              disabled={!name.trim() || !mermaidCode.trim() || isLoading}
